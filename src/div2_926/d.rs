...
impl Graph {
    pub fn new(p_size: usize, path_size: usize, start_from: usize) -> Graph {
        Graph {
            paths: Vec::with_capacity(path_size),
    ...
    pub fn get(&self, now_p: usize) -> impl Iterator<Item = &'_ PathType> {
        self.p[now_p].iter().map(move |x| &self.paths[*x])
    }
}
...
fn dfs(g: &Graph, now_p: usize, key: &mut i64, father: usize) -> i64 {
    let mut res = 1;
    for path in g.get(now_p) {
        if path.to == father {
            continue;
        }
        let v = dfs(g, path.to, key, now_p);
        res = (res * (v + 1)) % MODN;
    }
    *key = (*key + res) % MODN;
    res
