...
impl Solution {
    pub fn calculate_score(s: String) -> i64 {
        ...
        for (i, &x) in s.iter().enumerate() {
            p!(key, res);
            let now = 26 - (x - BASE);
            let k = key.entry(now).or_insert_with(|| vec![]);
            if k.is_empty() {
                key.entry(x - BASE + 1).or_insert_with(|| vec![]).push(i);
            } else {
                res += (i - k.pop().unwrap()) as i64;
            }
        }
        res
    }
...
#[cfg(any(feature = "local_build", feature = "local"))]
pub fn main() {
    println!("res:{}", Solution::calculate_score("abcdef".to_string()));
}
