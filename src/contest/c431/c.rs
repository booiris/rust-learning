...
impl Solution {
    pub fn maximum_coins(coins: Vec<Vec<i32>>, k: i32) -> i64 {
        let mut c = coins
            .into_iter()
            .map(|x| x.into_iter().map(|x| x as i64).collect::<Vec<_>>())
            .collect::<Vec<_>>();
        ...
        let len = c.len();
        while l < len {
            while r < len && c[r][1] - c[l][0] < k {
                now += (c[r][1] - c[r][0] + 1) * c[r][2];
                r += 1;
                res = res.max(now);
            }
            if r < len && c[r][0] - c[l][0] < k {
                res = res.max(now + (k - c[r][0] + c[l][0]) * c[r][2]);
            } else if r == len {
                break;
            }
            now -= (c[l][1] - c[l][0] + 1) * c[l][2];
            l += 1;
        }

        c.sort_unstable_by_key(|x| x[1]);

        let mut l = (len - 1) as i32;
        let mut now = 0;
        let mut r = (len - 1) as i32;
        while r >= 0 {
            while l >= 0 && c[r as usize][1] - c[l as usize][0] < k {
                now += (c[l as usize][1] - c[l as usize][0] + 1) * c[l as usize][2];
                l -= 1;
                res = res.max(now);
            }
            if l >= 0 && c[r as usize][1] - c[l as usize][1] < k {
                res = res.max(now + (k - c[r as usize][1] + c[l as usize][1]) * c[l as usize][2]);
            } else if l < 0 {
                break;
            }
            p!(now);
            now -= (c[r as usize][1] - c[r as usize][0] + 1) * c[r as usize][2];
            r -= 1;
        }
...
pub fn main() {
    let coins = [[1, 10, 3]];
    let k = 2;
    println!("res:{}", Solution::maximum_coins(to_2_vec(coins), k));
}
