    ...
    pub fn max_length(nums: Vec<i32>) -> i32 {
        ...
        for i in 0..nums.len() {
            for j in i..=nums.len() {
                ...
                for k in i..j {
                    prod *= nums[k];
                    gcdn = gcd(gcdn, nums[k]);
                    lcmn = lcmn * nums[k] / gcd(lcmn, nums[k]);
                    if prod == gcdn * lcmn {
                        p!(prod, gcdn, lcmn, i, j, k, res);
                        res = res.max((k - i + 1) as i32);
                    }
                }
            }
        }
...
impl Solution {
    pub fn max_length(nums: Vec<i32>) -> i32 {
        ...
        let mut mul = 1;
        let mut l = 0;
        for (r, &now) in nums.iter().enumerate() {
            while gcd(mul, now) != 1 {
                mul /= nums[l];
                l += 1;
            }
...
#[cfg(any(feature = "local_build", feature = "local"))]
pub fn main() {
    let nums = [1, 2, 3, 1, 4, 5, 1];
    println!("res:{}", Solution::max_length(nums.into()));
}
