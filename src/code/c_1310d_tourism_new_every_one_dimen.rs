...
impl<T: std::clone::Clone> TwoDimeVec<T> {
    pub fn new(n: usize, m: usize, default_v: T) -> Self {
        Self {
            data: vec![default_v; n * m],
            n,
            m,
        }
    }
    ...
    pub fn fill(&mut self, default_v: T) {
        self.data.fill(default_v)
    }

    pub fn get_mut(&mut self, i: usize, j: usize) -> &mut T {
        self.data.get_mut(self.m * i + j).unwrap()
    }
}

fn solve() {
    let (n, k) = (i!(usize), i!(usize));
    let mut g = TwoDimeVec::new(n + 1, n + 1, 0);
    ...
    for _ in 0..5000 as i32 {
        ...
        let mut dp = TwoDimeVec::new(k + 1, n + 1, i32::MAX / 2);
        *dp.get_mut(0, 1) = 0;
        for step in 1..=k {
            for i in 1..=n {
                for j in 1..=n {
                    if color[i] != color[j] {
                        let x = *dp.get(step - 1, j) + g.get(j, i);
                        *dp.get_mut(step, i) = *dp.get(step, i).min(&x);
                    }
                }
            }
        }
        res = res.min(*dp.get(k, 1));
    }
    wln!(res);
}
