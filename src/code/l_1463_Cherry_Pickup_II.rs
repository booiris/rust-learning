...
impl Solution {
    pub fn cherry_pickup(grid: Vec<Vec<i32>>) -> i32 {
        let n = grid.len();
        let m = grid[0].len();
        let mut dp = vec![vec![vec![-1; m + 5]; m + 5]; n + 5];
                ...
                for j in 1..=m {
                    for x in DIR {
                        for y in DIR {
                            ...
                            dp[k][i][j] = dp[k][i][j].max(dp[k - 1][nx][ny]);
                        }
                    }
                    if dp[k][i][j] == -1 {
                        continue;
                    ...
                    if i == j {
                        dp[k][i][j] += grid[k - 1][i - 1];
                    } else {
                        dp[k][i][j] += grid[k - 1][i - 1] + grid[k - 1][j - 1];
                    }
                }
            }
        }
