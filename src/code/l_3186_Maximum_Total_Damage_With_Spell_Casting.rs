...
pub fn main() {
    println!(
        "res:{}",
        Solution::maximum_total_damage([1, 1, 3, 4].into())
    );
}
...
impl Solution {
    pub fn maximum_total_damage(mut power: Vec<i32>) -> i64 {
        let mut res = 0;
        let mut pppre_max = (0, i32::MIN);
        let mut ppre_max = (0, i32::MIN);
        let mut pre_max = (0, i32::MIN);
        power.sort_unstable();
        let mut i = 0;
        let len = power.len();
        while i < len {
            let mut now = i;
            while now < len && power[now] == power[i] {
                now += 1;
            }
            let maxn = if pre_max.1 + 2 < power[i] {
                pre_max.0
            } else if ppre_max.1 + 2 < power[i] {
                ppre_max.0
            } else {
                pppre_max.0
            };

            let nown = maxn + power[i] as i64 * (now - i) as i64;

            p!(pppre_max, ppre_max, pre_max, nown, maxn);

            if ppre_max.0 >= pppre_max.0 {
                pppre_max = ppre_max;
            }

            if pre_max.0 >= ppre_max.0 {
                ppre_max = pre_max;
            }

            if nown >= pre_max.0 {
                pre_max = (nown, power[i]);
            }
            res = res.max(nown);
