...
struct NumberContainers {
    ...
    key: HashMap<i32, (BinaryHeap<Reverse<i32>>, HashMap<i32, i32>)>,
}
...
impl NumberContainers {
    ...
    fn change(&mut self, index: i32, number: i32) {
        let key = self
            .key
            .entry(number)
            .or_insert((BinaryHeap::new(), HashMap::new()));
        key.0.push(Reverse(index));
        if let Some(pre) = self.index.insert(index, number) {
            *self.key.get_mut(&pre).unwrap().1.entry(index).or_insert(0) += 1;
        }
    }

    fn find(&mut self, number: i32) -> i32 {
        if let Some(key) = self.key.get_mut(&number) {
            while let Some(top) = key.0.peek() {
                let index = top.0;
                if let Some(cnt) = key.1.get_mut(&index) {
                    if *cnt == 0 {
                        return index;
                    }
                    *cnt -= 1;
