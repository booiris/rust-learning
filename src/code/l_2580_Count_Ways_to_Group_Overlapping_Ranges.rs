...
impl Solution {
    pub fn count_ways(ranges: Vec<Vec<i32>>) -> i32 {
        let mut ranges = ranges.into_iter().map(|x| (x[0], x[1])).collect::<Vec<_>>();
...
pub fn main() {
    ...
    println!("res:{}", Solution::count_ways(to_2_vec(x)));
}
