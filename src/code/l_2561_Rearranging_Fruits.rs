...
pub fn main() {
    println!(
        "res:{}",
        Solution::min_cost(
            [84, 80, 43, 8, 80, 88, 43, 14, 100, 88].into(),
            [32, 32, 42, 68, 68, 100, 42, 84, 14, 8].into()
        )
    );
}
...
impl Solution {
    pub fn min_cost(basket1: Vec<i32>, basket2: Vec<i32>) -> i64 {
        ...
        let minc = *basket1.iter().min().min(basket2.iter().min()).unwrap();
        basket1.iter().for_each(|x| *k1.entry(x).or_insert(0) += 1);
        basket2.iter().for_each(|x| *k2.entry(x).or_insert(0) += 1);
        let mut key = k1.clone();
        for (k, v) in &k2 {
            *key.entry(k).or_insert(0) += v;
        }
        ...
        for (k, v) in key {
            ...
            let x = ((v / 2 - k1.get(k).unwrap_or(&0)) as i32).abs();
            if x != 0 {
                all += x;
                diff.push((*k, x));
            }
        ...
        let mut now = 0;
        for i in 0..diff.len() {
            if now + diff[i].1 > all / 2 {
                res += (all / 2 - now) as i64 * diff[i].0.min(minc * 2) as i64;
                break;
            }
            now += diff[i].1;
            res += diff[i].1 as i64 * diff[i].0.min(minc * 2) as i64;
            if now == all / 2 {
                break;
            }
        }
