...
impl Solution {
    pub fn min_non_zero_product(p: i32) -> i32 {
        ...
        let maxn = (qpow(2, p, MODN) + MODN - 1) % MODN;
        let base = (maxn + MODN - 1) % MODN;
        let top = (qpow(2, p - 1, MODN - 1) + MODN - 2) % (MODN - 1);
        (maxn * qpow(base, top, MODN) % MODN) as i32
    }
}

#[cfg(feature = "local")]
pub fn main() {
    println!("res:{}", Solution::min_non_zero_product(1000000000));
}
