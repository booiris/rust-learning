...
impl Solution {
    pub fn get_coprimes(nums: Vec<i32>, edges: Vec<Vec<i32>>) -> Vec<i32> {
        let len = nums.len();
        let mut key = vec![vec![false; 55]; 55];
        for i in 1..=50 {
            for j in 1..=i {
                if gcd(i, j) == 1 {
                    key[i][j] = true;
        ...
        let mut g = Graph::new(len + 5, 0);
        for e in edges {
            g.add_bi_path(e[0] as usize, e[1] as usize, 1);
        }
        let mut pre = vec![vec![]; 55];
        let mut res = vec![-1; len];
        dfs(&key, &mut pre, 0, 0, &g, &mut res, &nums, 0);
        res
    }
}
...
fn dfs(
    ...
    pre[now_v].push((now, depth));
    for p in g.get(now) {
        if p.to == father {
            continue;
        }
        dfs(key, pre, p.to, now, g, res, v, depth + 1);
    }
    pre[now_v].pop();
}
