...
#[allow(dead_code)]
impl _Graph {
    pub fn new(p_size: usize, start_from: usize) -> _Graph {
        unsafe {
            PATHS.clear();
...
fn find_longest_p(g: &_Graph, now_p: usize) -> usize {
...
impl Graph {
    fn new(n: i32, edges: Vec<Vec<i32>>) -> Self {
        let n = n as usize;
        let mut dis = [[i32::MAX / 3; 105]; 105];
        for e in edges {
            dis[e[0] as usize][e[1] as usize] = e[2];
        }
        for x in 0..n {
            dis[x][x] = 0;
        }
    ...
    fn add_edge(&mut self, edge: Vec<i32>) {
        ...
        let cost = edge[2] as i32;
        if cost > self.dis[from][to] {
            return;
        }
        for x in 0..self.n {
            for y in 0..self.n {
                self.dis[x][y] = min(self.dis[x][y], self.dis[x][from] + cost + self.dis[to][y]);
            }
        }
    }

    fn shortest_path(&self, node1: i32, node2: i32) -> i32 {
        if self.dis[node1 as usize][node2 as usize] == i32::MAX / 3 {
            return -1;
        } else {
            self.dis[node1 as usize][node2 as usize]
        }
    }
}
