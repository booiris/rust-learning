...
pub fn main() {
    println!(
        "res:{}",
        Solution::trap_rain_water(to_2_vec([
            [12, 13, 1, 12],
            [13, 4, 13, 12],
            [13, 8, 10, 12],
            [12, 13, 12, 12],
...
impl Solution {
    pub fn trap_rain_water(height_map: Vec<Vec<i32>>) -> i32 {
        ...
        let mut vis = vec![vec![false; height_map[0].len()]; height_map.len()];
        for i in 0..height_map.len() {
            for j in 0..height_map[0].len() {
                if i == 0 || i == height_map.len() - 1 || j == 0 || j == height_map[0].len() - 1 {
                    key.push((-height_map[i][j], i, j));
                    vis[i][j] = true;
                }
            }
        }
        ...
        let mut res = 0;
        while let Some((now, x, y)) = key.pop() {
            let now = -now;
            for d in DIR {
                let nx = x as i32 + d.0;
                let ny = y as i32 + d.1;
                if nx < 0
                    || nx >= height_map.len() as i32
                    || ny < 0
                    || ny >= height_map[0].len() as i32
                    || vis[nx as usize][ny as usize]
                {
                    continue;
                }
                if now > height_map[nx as usize][ny as usize] {
                    res += now - height_map[nx as usize][ny as usize];
                }
                vis[nx as usize][ny as usize] = true;
                key.push((
                    -height_map[nx as usize][ny as usize].max(now),
                    nx as usize,
                    ny as usize,
                ));
            }
