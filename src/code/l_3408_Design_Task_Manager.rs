...
impl TaskManager {
    fn new(tasks: Vec<Vec<i32>>) -> Self {
        ...
        for task in tasks {
            let (uid, task_id, priority) = (task[0], task[1], task[2]);
            manager.add(uid, task_id, priority);
        }
        manager
    }
    ...
    fn exec_top(&mut self) -> i32 {
        while let Some((priority, task_id)) = self.task.pop() {
            if let Some(&(user_id, info_priority)) = self.task_info.get(&task_id) {
