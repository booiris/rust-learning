...
impl Bitree {
    ...
    pub fn new(len: usize) -> Self {
        Bitree {
            tree: vec![i32::MAX; len + 1],
            val: vec![i32::MAX; len + 1],
            len,
        }
    }
    ...
    fn update(&mut self, index: usize, v: i32) {
        ...
        while pos <= self.len {
            ...
            let mut i = 1;
            while i < low {
                self.tree[pos] = min(self.tree[pos], self.tree[pos - i]);
                i <<= 1;
            }
            pos += Self::lowbit(pos);
        }
    ...
    pub fn query(&self, mut l: i32, mut r: i32) -> i32 {
        ...
        l = max(1, l);
        while r >= l {
            res = min(res, self.val[r as usize]);
            r -= 1;
            while r - Self::lowbit(r as usize) as i32 >= l {
                res = min(res, self.tree[r as usize]);
...
pub fn main() {
    println!(
        "res:{}",
        Solution::num_of_unplaced_fruits([4, 2, 5].into(), [3, 5, 4].into())
    );
}
...
impl Solution {
    pub fn num_of_unplaced_fruits(fruits: Vec<i32>, baskets: Vec<i32>) -> i32 {
        let mut key = vec![];
        for (i, x) in baskets.into_iter().enumerate() {
            key.push((x, i as i32));
        }
        key.sort_unstable();
        let len = key.len();
        let mut tree = Bitree::new(len);
        ...
        let mut res = 0;
        for v in fruits {
            let index = key.partition_point(|&x| x.0 < v);
            let aim = tree.query((index + 1) as i32, len as i32);
            if aim == i32::MAX {
                res += 1;
            } else {
                let index = m[&aim];
                tree.update(index, i32::MAX);
            }
        }
        res
    }
