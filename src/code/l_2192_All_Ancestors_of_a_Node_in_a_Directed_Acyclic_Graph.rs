...
impl Solution {
    pub fn get_ancestors(n: i32, edges: Vec<Vec<i32>>) -> Vec<Vec<i32>> {
        ...
        for e in edges {
            g.add_path(e[0] as usize, e[1] as usize, 1);
        }
        let mut father = vec![vec![]; n];
        ...
        for i in 0..n {
            ...
            vis[i] = true;
            dfs(i, &g, &mut father, &mut vis);
        }
        for i in 0..n {
            let mut vis = vec![false; n];
            let mut now = father[i].clone();
            let mut temp = vec![];
            while let Some(v) = now.pop() {
                if vis[v] {
                    continue;
                }
                vis[v] = true;
...
fn dfs(now: usize, g: &Graph, father: &mut Vec<Vec<usize>>, vis: &mut Vec<bool>) {
    for p in g.get(now) {
        father[p.to].push(now);
        if vis[p.to] {
            continue;
        }
        vis[p.to] = true;
        dfs(p.to, g, father, vis);
    }
}
