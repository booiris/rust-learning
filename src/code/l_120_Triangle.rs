...
impl Solution {
    pub fn minimum_total(triangle: Vec<Vec<i32>>) -> i32 {
        let mut dp = vec![vec![i32::MAX / 2; triangle.len() + 1]; triangle.len() + 1];
        dp[0][0] = 0;
        for i in 0..triangle.len() {
            for j in 0..triangle[i].len() {
                dp[i + 1][j] = dp[i + 1][j].min(dp[i][j] + triangle[i][j]);
                dp[i + 1][j + 1] = dp[i + 1][j + 1].min(dp[i][j] + triangle[i][j]);
            }
        }
        *dp[triangle.len()].iter().min().unwrap()
    }
}
