...
fn solve(dis: &[i32], start_pos: i32, k: i32, sumn: &[i32]) -> i32 {
    let mut res = 0;
    let s_have = dis.partition_point(|&x| x < start_pos);
    let s_no = dis.partition_point(|&x| x <= start_pos);
    for i in 0..dis.len() {
        let rest = k - (start_pos - dis[i]).abs();
        if rest < 0 {
            continue;
        }
        let key = dis[i] + rest;
        let index = dis.partition_point(|&x| x <= key);
        let temp = if s_have > i {
            if index > s_have {
                sumn[index] - sumn[i]
            } else {
                sumn[s_no] - sumn[i]
...
impl Solution {
    pub fn max_total_fruits(fruits: Vec<Vec<i32>>, start_pos: i32, k: i32) -> i32 {
        let mut sumn = vec![0; fruits.len() + 1];
        let mut dis = vec![0; fruits.len()];
        for i in 0..fruits.len() {
            sumn[i + 1] = sumn[i] + fruits[i][1];
            dis[i] = fruits[i][0];
        }
        let mut res = solve(&dis, start_pos, k, &sumn);
        for i in 0..fruits.len() {
            sumn[i + 1] = sumn[i] + fruits[fruits.len() - i - 1][1];
            dis[i] = -fruits[fruits.len() - i - 1][0];
        }
        res = res.max(solve(&dis, -start_pos, k, &sumn));
        res
    }
}
