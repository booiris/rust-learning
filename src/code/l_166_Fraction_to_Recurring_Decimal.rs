...
impl Solution {
    pub fn fraction_to_decimal(numerator: i32, denominator: i32) -> String {
        ...
        let a = if numerator >= denominator {
            let key = numerator / denominator;
            numerator -= key * denominator;
            key
        } else {
            0
        };
        ...
        while numerator != 0 {
            ...
            numerator *= 10;
            if let Some(index) = key.get(&numerator) {
                loop_start = *index;
                break;
            }
            key.insert(numerator, res.len());
            let b = numerator / denominator;
            numerator -= b * denominator;
            res.push((b as u8 + '0' as u8) as char);
        }
        let mut ans = format!("{}{}", is_neg, a);
        if res.len() > 0 {
            ans.push('.');
