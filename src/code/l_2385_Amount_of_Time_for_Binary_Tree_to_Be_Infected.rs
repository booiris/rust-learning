...
impl Solution {
    pub fn amount_of_time(root: Option<Rc<RefCell<TreeNode>>>, start: i32) -> i32 {
        ...
        if let Some(root) = root {
            ...
            let lv = if let Some(l) = root.borrow().left.clone() {
                dfs(l, start, &mut res, &mut flag1, 1)
            } else {
                0
            };
            let mut flag2 = 0;
            let rv = if let Some(r) = root.borrow().right.clone() {
                dfs(r, start, &mut res, &mut flag2, 1)
            } else {
                0
            };
            if flag1 != 0 {
                res = (rv + flag1).max(res);
            } else if flag2 != 0 {
                res = (lv + flag2).max(res);
            } else {
                res = rv.max(lv);
            }
        }
...
fn dfs(now: Rc<RefCell<TreeNode>>, start: i32, res: &mut i32, flag: &mut i32, depth: i32) -> i32 {
    let val = now.borrow().val;
    let mut now_d = 0;
    if let Some(left) = now.borrow().left.clone() {
        let v = dfs(left, start, res, flag, depth + 1);
        now_d = now_d.max(v);
    }
    if let Some(right) = now.borrow().right.clone() {
        let v = dfs(right, start, res, flag, depth + 1);
        now_d = now_d.max(v);
    }
    if val == start {
        *flag = depth;
