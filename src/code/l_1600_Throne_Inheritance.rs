...
impl ThroneInheritance {
    fn new(king_name: String) -> Self {
        let g = Graph::new(1e5 as usize + 5, 0);
        let mut name = HashMap::new();
        let mut id = HashMap::new();
        name.insert(0, king_name.clone());
        id.insert(king_name, 0);
        Self {
            g,
            name,
            now: 1,
    ...
    fn birth(&mut self, parent_name: String, child_name: String) {
        self.name.insert(self.now, child_name.clone());
        self.id.insert(child_name.clone(), self.now);
        self.now += 1;
        self.g
            .add_path(self.id[&parent_name], self.id[&child_name], 1);
    }

    fn death(&mut self, name: String) {
        self.dead[self.id[&name]] = true;
    }

    fn get_inheritance_order(&self) -> Vec<String> {
        let mut res = vec![];
        self.dfs(0, &mut res);
        res.iter().map(|x| self.name[x].clone()).collect()
    }

    fn dfs(&self, now: usize, res: &mut Vec<usize>) {
        if !self.dead[now] {
            res.push(now);
        }
        for p in self.g.get(now) {
            self.dfs(p.to, res);
        }
    }
}
