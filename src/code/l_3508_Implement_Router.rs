...
impl Router {
    ...
    fn add_packet(&mut self, source: i32, destination: i32, timestamp: i32) -> bool {
        ...
        if self.in_q.contains(&pack) {
            return false;
        }
        if self.in_q.len() as i32 >= self.limit {
            self.forward_packet();
        }
        self.data.push_back(pack);
        self.in_q.insert(pack);
        self.dest
    ...
    fn forward_packet(&mut self) -> Vec<i32> {
        ...
        let res = self.data.pop_front().unwrap();
        self.in_q.remove(&res);
        self.dest.get_mut(&res.destination).unwrap().pop_front();
        vec![res.source, res.destination, res.timestamp]
    }

    fn get_count(&self, destination: i32, start_time: i32, end_time: i32) -> i32 {
        if let Some(data) = self.dest.get(&destination) {
            (data.partition_point(|&x| x <= end_time) - data.partition_point(|&x| x < start_time))
                as i32
        } else {
            0
        }
