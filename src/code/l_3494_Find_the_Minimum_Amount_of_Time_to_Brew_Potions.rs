...
pub fn main() {
    println!(
        "res:{}",
        Solution::min_time([1, 5, 2, 4].into(), [5, 1, 4, 2].into())
    );
}

impl Solution {
    pub fn min_time(skill: Vec<i32>, mana: Vec<i32>) -> i64 {
        let mut end = vec![0; skill.len()];
        for x in mana {
            let mut start_time = 0;
            let mut sum = 0;
            for i in 0..skill.len() {
                start_time = start_time.max(end[i] - sum);
                sum += skill[i] as i64 * x as i64;
            }
            sum = 0;
            for i in 0..skill.len() {
                sum += skill[i] as i64 * x as i64;
                end[i] = start_time + sum;
            }
        }
        end[skill.len() - 1]
    }
