...
impl Solution {
    pub fn cherry_pickup(grid: Vec<Vec<i32>>) -> i32 {
        let n = grid.len();
        let mut dp = vec![vec![-1; n + 5]; n + 5];
        dp[1][1] = grid[0][0];
        for k in 2..=2 * n - 1 {
            for x1 in (k.checked_sub(n - 1).unwrap_or(1).max(1)..=k.min(n)).rev() {
                for x2 in (k.checked_sub(n - 1).unwrap_or(x1).max(x1)..=k.min(n)).rev() {
                    if grid[x1 - 1][k - x1] == -1 || grid[x2 - 1][k - x2] == -1 {
                        dp[x1][x2] = -1;
                        continue;
                    }
                    ...
                    if x1 == x2 {
                        dp[x1][x2] += grid[x1 - 1][k - x1];
                    } else {
                        dp[x1][x2] += grid[x1 - 1][k - x1] + grid[x2 - 1][k - x2];
                    }
                }
            }
        }
...
#[cfg(feature = "local")]
pub fn main() {
    let x = to_2_vec([[0, 1, -1], [1, 0, -1], [1, 1, 1]]);
    println!("res:{}", Solution::cherry_pickup(x));
}
