...
impl Solution {
    pub fn maximum_binary_string(binary: String) -> String {
        let mut ini = binary.chars().collect::<Vec<_>>();
        let len = ini.len();
        let mut index = 0;
        let mut last1 = 0;
        while index < len - 1 {
            if ini[index] == '0' {
                if ini[index + 1] == '0' {
                    ...
                    let mut t_index = last1.max(index + 1);
                    while t_index < len && ini[t_index] == '1' {
                        t_index += 1;
                    }
                    if t_index < len {
                        ini[index] = '1';
        ...
        String::from_utf8(ini.into_iter().map(|x| x as u8).collect()).unwrap()
    }
}

#[cfg(any(feature = "local_build", feature = "local"))]
pub fn main() {
    println!("res:{}", Solution::maximum_binary_string("01111001100000110010".into()));
}
