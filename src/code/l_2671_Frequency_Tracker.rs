...
impl FrequencyTracker {
    ...
    fn add(&mut self, number: i32) {
        if let Some(num_freq) = self.nums.get(&number) {
            self.freq.get_mut(&num_freq).map(|x| x.remove(&number));
        }
        let entry = self.nums.entry(number).or_default();
        *entry += 1;
        self.freq.entry(*entry).or_default().insert(number);
    }
