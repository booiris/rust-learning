...
impl Solution {
    pub fn combination_sum(candidates: Vec<i32>, target: i32) -> Vec<Vec<i32>> {
        let mut key: Vec<Vec<Vec<i32>>> = vec![vec![]; target as usize + 5];
        key[0].push(vec![]);
        for x in candidates {
            for i in x..=target {
                for j in 0..key[(i - x) as usize].len() {
                    let mut j = key[(i - x) as usize][j].clone();
                    j.push(x);
                    key[i as usize].push(j);
                }
            }
        }
        std::mem::take(&mut key[target as usize])
    }
}
...
pub fn main() {
    ...
    println!("res:{:?}", Solution::combination_sum(x, 7));
}
