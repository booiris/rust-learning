...
struct MovieRentingSystem {
    valid: HashMap<i32, BTreeSet<(i32, i32)>>,
    mv_price: HashMap<Data, i32>,
    retend: BTreeSet<PriceData>,
}
...
impl MovieRentingSystem {
    fn new(_: i32, entries: Vec<Vec<i32>>) -> Self {
        ...
        for entry in entries {
            ...
            let data = Data { shop, movie };
            res.valid.entry(movie).or_default().insert((price, shop));
            res.mv_price.insert(data, price);
        }
        res
    }
    ...
    fn search(&mut self, movie: i32) -> Vec<i32> {
        self.valid
            .get(&movie)
            .map_or_else(Vec::new, |x| x.iter().take(5).map(|x| x.1).collect())
    }

    fn rent(&mut self, shop: i32, movie: i32) {
        let data = Data { shop, movie };
        let price = *self.mv_price.get(&data).unwrap();
        self.valid.get_mut(&movie).unwrap().remove(&(price, shop));
        self.retend.insert(PriceData { price, movie, shop });
    }

    fn drop(&mut self, shop: i32, movie: i32) {
        let data = Data { shop, movie };
        let price = *self.mv_price.get(&data).unwrap();
        self.retend.remove(&PriceData { price, shop, movie });
        self.valid.get_mut(&movie).unwrap().insert((price, shop));
    }
